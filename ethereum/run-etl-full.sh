#!/bin/bash
WORK_DIR=${WORK_DIR:-"/home/ubuntu/onchain-etl/ethereum"}; source $WORK_DIR/.env
...
SQL_TEMPLATE=${SQL_TEMPLATE:-"sql-export"}
...
log "Wait for all background processes to finish"
for pid in "${pids[@]}"; do
 wait $pid
done
